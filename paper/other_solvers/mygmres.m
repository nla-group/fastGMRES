function [x,flag,relres,iter,resvec,restime] = mygmres(A,b,restart,tol,maxit,M1,M2,maxtime,varargin)
%GMRES   Generalized Minimum Residual Method.
% 
%   Modified version adding a timer and timeout functionality.
%   initial guess is always zero, and maxtime is provided in place of x0
%
...
tstart = tic;
flag =[]; % assign to have value in case of timeout
relres=[];
iter = [];
resvec=[];
restime=[];
...
restime = zeros(inner*outer+1,1); % SG
resvec(1) = normr;                % resvec(1) = norm(b-A*x0)
restime(1) = toc(tstart);         % SG
...
for outiter = 1 : outer
    ...
    for initer = 1 : inner
        ...
        restime((outiter-1)*inner+initer+1) = toc(tstart);         % SG
        normr_act = normr;
        ...
        if (normr <= tolb || stag >= maxstagsteps || moresteps)
            ...
            restime((outiter-1)*inner+initer+1) = toc(tstart);
...
restime = restime(1:max(outiter-1,0)*inner+initer+1);
if flag == 2 && initer ~= 0
    resvec(end) = [];
    restime(end) = [];
end
