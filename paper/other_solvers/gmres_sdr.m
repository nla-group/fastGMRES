...
for restart = 1:param.max_restarts
    ...
    [ e, r, cycle_out ] = srgmres_cycle(A, r, param, x, b);
	...
	timevec(restart+1) = toc(tstart);
    iters(restart+1) = cycle_out.m;

    sres = [ sres , cycle_out.sres ];

    if param.verbose
        fprintf('  restart = %2d, ||r|| = %5.2e\n',restart, residnorm)
    end
    ...
    if norm(r) < param.tol || toc(tstart) > param.maxtime
        break
    end
...
function [e,r,out] = srgmres_cycle(A,r0,param,x,b)
